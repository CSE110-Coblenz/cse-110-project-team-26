...
function convertCoordToKonva(coord: number, isX: boolean): number {
    if (isX) {
        //return OFFSET + (coord - getXMin() + 1) * PIX_PER_UNIT;
        return OFFSET + (coord - -6 + 1) * PIX_PER_UNIT;
    }
    return OFFSET + (7 - coord + 1) * PIX_PER_UNIT;
    //return OFFSET + (getYMax() - coord + 1) * PIX_PER_UNIT;
}
